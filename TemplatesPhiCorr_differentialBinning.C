...
void SignalSwapOnly()
{
  ...
  // TFile *outfile = new TFile("templates_out_12Nov.root", "recreate");
    ...
    TString fits_file_path = "TF1_outputs_12Nov_02.root";
   ...
   TH2D *B1B2Template = CreateTemplate(background1, background2, B1B2Mass);
    ...
    temp_canvas->SaveAs("pdfs/templates_12Nov.root");
    */
    ...
    */
    ...
    {
        /*
        if (fit_range_low < 1.55 || fit_range_high > 2.2)
            {
                break;
            }
        else
            {cout << "fit low = " << fit_range_low << " & high = " << fit_range_high << endl;
            }
        */
        ...
        */
        cout << "iteration = " << endl;
        //M1M2MassClone->GetXaxis()->SetRangeUser(fit_range_low, fit_range_high);
        //M1M2MassClone->GetYaxis()->SetRangeUser(fit_range_low, fit_range_high);
        for (int i = 0; i < 10; i++)
        {
            cout << i << endl;
