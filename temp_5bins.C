...
//const char *file_path = "/scratch/bell/awesole/analysis_hepmc_06_Nov_2024/mergedfile.root";
...
int temp_5bins(float prob_cutoff, TString outfile)
{
...
int main(int argc, char* argv[]) {
        if (argc != 3) {
                std::cerr << "Usage: " << argv[0] << " <prob_cutoff> <outfile>" << std::endl;
                return 1;
        }
        float prob_cutoff = std::atof(argv[1]);
        TString outfile = argv[2];

        temp_5bins(prob_cutoff, outfile);
        return 0;
}
